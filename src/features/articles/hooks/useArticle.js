...
const useArticle = () => {
    ...
    /**FETCH ARTICLES */
    const fetchArticlesData = useCallback( async({page=1, per_page=12})=>{
          
        dispatch({type:ARTICLE_ACTIONS.FETCH_START})

        try {

            const articles = await articleService.getAllArticles({page,per_page})
            ...
            toast.success("Articles loaded successfully",{duration:5000,position:"top-right"})
            return true
